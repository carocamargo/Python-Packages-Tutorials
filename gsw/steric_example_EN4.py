...
max_depth = np.array([depth[np.where(np.isnan(salinity[:,icoord]))[0][0]]
...
# plot
plt.figure()
plt.pcolor(max_depth,
           cmap = cm.deep,
           vmin=np.nanmin(max_depth),vmax=np.nanmax(max_depth))
...
temperature = np.array((ds.temperature[itime]-273.15)*mask).reshape(dimdepth,dimlat*dimlon)

# make an empty array for steric height
ste_h = np.full_like( 
    np.zeros((dimtime,dimdepth,dimlat*dimlon)),
                np.nan)
...
plt.figure()
plt.title('Steric SL')
plt.pcolor(ste_h[0,0,:].reshape(dimlat,dimlon),
           cmap=cm.thermal,
            vmin=-2,vmax=5,
           )
